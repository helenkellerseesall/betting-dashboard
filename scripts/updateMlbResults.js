...
/**
 * MLB tracked-result ingestion CLI.
 *
 * Usage:
 *   # 1) Provide a JSON file mapping bet ids to results:
 *   node scripts/updateMlbResults.js --date=2026-05-05 --bets=path/to/bets.json --legs=path/to/legs.json
 *
 *   # 2) Provide a single bet/leg result on the command line:
 *   node scripts/updateMlbResults.js --date=2026-05-05 --bet="<id>=win"
 *   node scripts/updateMlbResults.js --date=2026-05-05 --leg="<legId>=loss"
 *
 *   # 3) Pipe a JSON object on stdin:
 *   echo '{"bets":{"<id>":"win"},"legs":{"<legId>":"loss"}}' | node scripts/updateMlbResults.js --date=2026-05-05
 *
 * Result values: "win" | "loss" | "push" | "void" | "pending"
 *
 * The script ONLY updates the daily files in
 *   backend/runtime/tracking/mlb_tracked_bets_<date>.json
 *   backend/runtime/tracking/mlb_tracked_slips_<date>.json
 * and is intentionally separate from the live pipeline.
 */

const fs = require("fs")
const path = require("path")
const { applyResults } = require("../backend/pipeline/mlb/phase4Tracking")
...
async function main() {
  ...
  console.log("[updateMlbResults] applied:", JSON.stringify(result, null, 2))
  console.log("[updateMlbResults] inputs:", JSON.stringify({
    date: args.date,
  ...
  if (args.runFull) {
    ...
    console.log("[updateMlbResults] full nightly:", JSON.stringify({
      ok: nightly.ok,
  ...
  if (args.runReview) {
    const out = runPostGameReview({ sport: "mlb", date: args.date, actuals: args.actuals })
    console.log("[updateMlbResults] review:", JSON.stringify({
      counts: out.counts,
...
main().catch((err) => {
  console.error("[updateMlbResults] error:", err?.message || err)
  process.exitCode = 1
})
