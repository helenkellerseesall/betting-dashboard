  ...
  return (
    <div>
      <h2 className="ws-section-title">
        Process Review <small>{days}-day window</small>
      </h2>
      ...
      {data && (
        <>
          <div className="ws-grid ws-grid-4" style={{ marginBottom: 14 }}>
            <KPI label="Bankroll" value={data.bankroll?.current != null ? `$${fmtNum(data.bankroll.current, 2)}` : "—"} sub={data.bankroll?.starting != null ? `start $${fmtNum(data.bankroll.starting, 2)}` : ""} />
            <KPI label="Profit" value={data.totals?.profit != null ? `${data.totals.profit >= 0 ? "+" : ""}$${fmtNum(data.totals.profit, 2)}` : "—"} sub={data.totals?.bets ? `${data.totals.bets} bets` : ""} />
            ...
            <KPI label="Avg CLV" value={data.report?.clv?.avgClv != null ? `${(data.report.clv.avgClv * 100).toFixed(2)}¢` : "—"} sub={data.report?.clv?.beatRate != null ? `${fmtPct(data.report.clv.beatRate, 1)} beat market` : ""} />
          </div>

          <div className="ws-grid ws-grid-2" style={{ marginBottom: 14 }}>
            <div className="ws-card">
              <strong>Good Process / Bad Process</strong>
