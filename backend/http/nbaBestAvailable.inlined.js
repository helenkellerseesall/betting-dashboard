  ...
  let nbaOpportunityBoard = null
  let nbaInsightBoard = null
  try {
    nbaOpportunityBoard = buildNbaOpportunityBoard({
      ladderBoard,
      corePropsBoard,
      completeUniverse,
    })
    nbaInsightBoard = buildNbaInsightBoard({
      ladderBoard,
      corePropsBoard,
      specialBoard,
      firstBasketBoard,
    ...
    console.log("[NBA BOARD PIPELINE ERROR]", e?.message || e)
    nbaOpportunityBoard = null
    nbaInsightBoard = null
  }

  console.log("[BEST AVAILABLE DEBUG]", {
    hasSnapshot: !!currentSnapshot,
    props: currentSnapshot?.props?.length
  })

  // Surface trackingSummary at the top level for ergonomic consumer access.
  // (It also lives at nbaOpportunityBoard.bestBetsBoard.trackingSummary.)
  const trackingSummary = nbaOpportunityBoard?.bestBetsBoard?.trackingSummary || null
