...
function getAvailablePrimarySlateRows(rows) {
  ...
  const nowMs = Date.now()
  // Session AW — Slate Integrity Repair V1: PREGAME events only.
  // The fetcher (buildSlateEvents) now filters out completed/in-progress games,
  // but a stale snapshot persisted on disk may still contain past events. This
  // defensive filter ensures the workstation NEVER surfaces props for a game
  // whose commence_time is in the past, regardless of snapshot freshness.
  const pregameEvents = scheduledEvents.filter((e) => {
    const t = e?.commence_time || e?.gameTime || e?.startTime || e?.start_time || e?.game_time || ""
    const ms = new Date(t).getTime()
    return Number.isFinite(ms) && ms > nowMs
  })
  const completedDroppedDefensive = scheduledEvents.length - pregameEvents.length
  if (completedDroppedDefensive > 0) {
    console.log("[SLATE-INTEGRITY] defensive past-event filter dropped " + completedDroppedDefensive + " of " + scheduledEvents.length + " events from oddsSnapshot.events")
  }
  const currentPregameGameCount = pregameEvents.length
  ...
  const primarySlateRows = primarySlateInputRows.filter((row) => {
    ...
    // Session AW — Slate Integrity Repair V1: when no PREGAME events exist
    // (slate is empty or all completed), reject ALL rows. Previously when
    // scheduledEventIdSet was empty we accepted everything (line "return true"),
    // which let stale rows through whenever the snapshot's events list was
    // empty. Now: empty pregame set ⇒ slate is genuinely empty ⇒ no rows pass.
    if (scheduledEventIdSet.size === 0) {
      logRowCheck(row, "no_pregame_events_in_slate")
      return false
    }

    const inScheduledSet = scheduledEventIdSet.has(String(row.eventId))
    if (inScheduledSet) {
      // Session AW: defensive per-row commence_time check. Even if the row's
      // event is in the pregame set, double-check the row's own gameTime/commence_time
      // because some snapshot rows may carry an older timestamp.
      const rowTime = row?.gameTime || row?.commence_time || null
      if (rowTime) {
        const ms = new Date(rowTime).getTime()
        if (Number.isFinite(ms) && ms <= nowMs) {
          logRowCheck(row, "row_event_already_started")
          return false
        }
      }
      return true
    ...
    // Session AW: REMOVED "active slate relaxation". That clause previously
    // accepted rows whose eventId was NOT in scheduledEventIdSet whenever the
    // slate was "active" — a major source of stale-prop leakage. Strict policy
    // now: row must be in the pregame event set, period.
...
// `logBestStage` was removed in a prior commit but six callers remain inside
// the /refresh-snapshot/hard-reset route (lines ~20236–20501). The bug was
// LATENT — pre-Session-AX hard-reset returned 404 before reaching the callers
// when scheduledEvents was empty (the common case once today's slate ended).
// Session AX correctly admits tomorrow's pregame games → execution now
// reaches the callers → ReferenceError: logBestStage is not defined → snapshot
// rebuild aborts → bestProps stays empty.
//
// Restored from git history (commit 3dd8ae5). Pure observability helper —
// returns its input unchanged so it composes safely in pipeline chains.
...
app.get("/refresh-snapshot/hard-reset", async (req, res) => {
  ...
  try {
    ...
    // Session AY — Hard-Reset Runtime Regression Fix.
    // The previous inline snapshot assembly (≈1500 lines below this point)
    // called multiple helpers that were removed in prior commits:
    //   logBestStage, bestPropsCompositeScore, buildBestPropsBalancedPool,
    //   diversifyByTeam, ensureBestPropsBookPresence,
    //   ensureBestPropsPlayableBookFloor, countByBookForRows, getPlayerAvgMin,
    //   getPlayerEdge, getPlayerHitRate, getPlayerInjuryRisk, getPlayerMinFloor,
    //   getPlayerMinStd, getPlayerMinutesRisk
    // The bug was LATENT — pre-Session-AX hard-reset returned 404 before
    // reaching these callers when scheduledEvents was empty (common case once
    // today's slate ended). Session AX correctly admits tomorrow's pregame
    // games → execution now reaches the callers → ReferenceError → snapshot
    // rebuild aborts → bestProps stays empty.
    //
    // Smallest viable fix: delegate to the SAME handleNbaRefreshSnapshotAfterMlbBranch
    // handler that the working /refresh-snapshot route already uses. This:
    //   - eliminates ALL 13+ missing-function risks at once
    //   - reuses the proven snapshot-generation code path
    //   - preserves the unique cache-clearing benefit of hard-reset (done above)
    //   - inherits Session AW slate-truth integrity (no stale games leak)
    //   - inherits Session AX future-slate acceptance (tomorrow's games admitted)
    ...
    console.log("[HARD-RESET-DELEGATED]", {
      events: Array.isArray(oddsSnapshot?.events) ? oddsSnapshot.events.length : 0,
      props: Array.isArray(oddsSnapshot?.props) ? oddsSnapshot.props.length : 0,
      bestProps: Array.isArray(oddsSnapshot?.bestProps) ? oddsSnapshot.bestProps.length : 0,
    })
