...
check("pre-reg: opposition ρ from COMMITTED verdicts, PASS-only, abstain-on-unresolvable", /oppo\.verdict === "PASS"/.test(src) && /if \(!OPPO_ON\) return null/.test(src) && /if \(!opp\) return null/.test(src))
...
check("gates: per-column tally w/ legacy rawFlag default-true + counterfactual kill bar", /e\.rawFlag !== false/.test(src) && /COUNTERFACTUAL KILL BAR/.test(src) && /declines majority of raw losses/.test(src) && /cureGates: \{ A: gateTally\("A"\), B: gateTally\("B"\), C: gateTally\("C"\) \}/.test(src))
