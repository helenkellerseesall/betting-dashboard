 ...
 *   • EXPECTED STATISTICS leaderboard → xwOBA, xBA, xSLG (+ wOBA)
 *   • EXIT VELOCITY & BARRELS leaderboard → barrel%, hard-hit%, avg EV
 * Joined by canonNameKey. Parsed by headers.indexOf with CANDIDATE-NAME lists (Savant column names
...
const MIN = "q"   // qualified batters (tunable: "q" | a PA number). Qualified → sane season aggregates.

// Savant leaderboard CSV endpoints (csv=true → one row per qualified player).
const URL_EXPECTED = `https://baseballsavant.mlb.com/leaderboard/expected_statistics?type=batter&year=${SEASON}&position=&team=&filterType=bip&min=${MIN}&csv=true`
const URL_EXITVELO = `https://baseballsavant.mlb.com/leaderboard/exit_velocity_barrels?type=batter&year=${SEASON}&position=&team=&min=${MIN}&csv=true`
...
async function main() {
  console.log("[statcast-quality] season (ET, from slateDate.currentSlateDateEt):", SEASON, "| min:", MIN)
  console.log("[statcast-quality] EXPECTED url:", URL_EXPECTED)
  console.log("[statcast-quality] EXITVELO url:", URL_EXITVELO)

  const exp = await fetchCsv(URL_EXPECTED)
  const ev = await fetchCsv(URL_EXITVELO)
  if (!exp.ok && !ev.ok) {
    console.error(`[statcast-quality] BOTH leaderboards failed (expected: ${exp.status || exp.err}, exitvelo: ${ev.status || ev.err}).`)
  ...
  const evP = ev.ok ? parseLeaderboardCsv(ev.csv, EV_COLS) : { byKey: {}, header: [], resolved: {}, players: 0 }
  ...
  console.log("[statcast-quality] EXITVELO header:", JSON.stringify(evP.header))
  console.log("[statcast-quality] EXITVELO column mapping (field→actual col):", JSON.stringify(evP.resolved))

  // Join by canonNameKey (union of both leaderboards).
  const keys = new Set([...Object.keys(expP.byKey), ...Object.keys(evP.byKey)])
  const out = {}
  for (const k of keys) {
    const e = expP.byKey[k] || {}, v = evP.byKey[k] || {}
    out[k] = {
      barrelPct: v.barrelPct ?? null,            // percent (0–100), per batted ball
      hardHitPct: v.hardHitPct ?? null,          // percent (0–100)
      avgExitVelocity: v.avgExitVelocity ?? null, // mph
      xwoba: e.xwoba ?? null,                     // decimal
      xba: e.xba ?? null,
      xslg: e.xslg ?? null,
      woba: e.woba ?? null,
      pa: e.pa ?? null,
      battedBalls: v.battedBalls ?? null,
      source: "baseball_savant_leaderboard(expected_statistics+exit_velocity_barrels)",
      season: SEASON,
  ...
  console.log(`\n[statcast-quality] players (union): ${names.length}  | expected rows: ${expP.players}  exitvelo rows: ${evP.players}`)
  console.log(`[statcast-quality] coverage — barrelPct ${cov("barrelPct")}/${names.length} · hardHitPct ${cov("hardHitPct")}/${names.length} · xwoba ${cov("xwoba")}/${names.length} · xslg ${cov("xslg")}/${names.length} · xba ${cov("xba")}/${names.length} · avgEV ${cov("avgExitVelocity")}/${names.length}`)
  ...
  for (const n of names.slice(0, 8)) console.log(`  ${n.padEnd(26)} barrel%=${out[n].barrelPct} hardHit%=${out[n].hardHitPct} xwoba=${out[n].xwoba} avgEV=${out[n].avgExitVelocity}`)
  console.log("[statcast-quality] wrote", OUT, "— STAGING, zero live consumer by design (wire post-freeze)")
  console.log("[statcast-quality] SUCCESS BAR: barrelPct + hardHitPct + xwoba must show ≥80% coverage AND sane medians. If any column mapped to null above, tell CB the real header name to add as an alias.")
}
