...
const SUITES = [
  ...
  "verifyIntakeFixes", // INTAKE-FIXES (2026-07-29, ASK 147e814; the 1:56 AM FanDuel Ks parlay rejected twice) — never-future bet-date measured against CALENDAR ET not the 4 AM slate day (window pinned: calendar-today recordable while the slate lags; tomorrow still rejected; slate stays the DEFAULT date), stat alias map at the ONE owner canonMlbStat (Strikeouts→ks etc., both cores, unknown still fails honestly), FE record-flow ingest normalization (confirm modal shows what validates; analyze-tab display map untouched), lookup-date pin (game-date-keyed join needed no change); real-CLI dry e2e.
  "verifyIncidentPack", // INCIDENT+ACCOUNTING (2026-07-30, ASK 7aae50f) — ROOT: fetchMlbGameResults.normName NFD-strips (accented players graded again; the 289-row 7/23 class; FirstHr parity) · parlay finals-fallback for twin-less legs (sync settle + async CLI prefetch cache, provenance notes, scratch-rule-mirror void, never-guess; hermetic e2e settles the Clement class LOSS) · parlaySettle bar: pending past a GRADED slate = RED, no grace · effective-loss lens (deadIds in betRollup, irreversible-only, fail-open, deduped jsonl) + legDeathDisagreement alarm (count 23) · TOMORROW PREVIEW banner.
  "verifyGraduationBoard", // GRADUATION BOARD (2026-07-30, ASK f5ee1b6) — one read-only aggregator over existing artifacts (family exams incl. never-examined honesty, TB/RBIs re-point verdicts verbatim, rung/cures/parlay gates w/ named unlocks, queued prior refusing fake progress), /status section + /m compact section + sidecar route, stall alarm #24 (N=2 pinned to the REAL 7/25-27 plateau replayed in the fixture; recomputes from raw artifacts never the sidecar), Sunday board+receipts synthesis, trend "too new" honesty + exact 7-slate delta; hermetic tmp-dir e2e.
  "verifyValidatorAxisB", // VALIDATOR CRASH FIX (ASK 37a63a6) — d7a448e deleted the norm helper but missed the Axis-B key-builder: every exam run since 07-21 died pre-write while the matrix stayed green (no fixture executed Axis-B). NFD key normalizer restored (resolution stays playerNameJoin); this fixture RUNS the real validator over a synthetic world WITH a ladder file (accented player, deduped rung) and asserts exit 0 + verdicts JSON written + rung counted — the blind spot is load-bearing in the matrix now.
  "verifyVoidWait", // VOID-WAIT RECONCILIATION (2026-08-02, ASK 70cf06c; b62d25d6) — classifyLegs = the ONE void-candidacy authority (graded / void_candidate only from PRESENT finals / unresolved never guessed; accent-safe), parlaySettle v3 waiting-not-stale until the 2-day window arms (cry-wolf killed), effective-WIN mirror (informational, toWin stays pending, bidirectional disagreement polices it), per-leg price persistence (route+FE — the §10 defer class ends at record time), f3d58e16 class pinned (void+LOSS ⇒ auto LOSS, no deferral); hermetic e2e.
