...
| Barrel% · Hard-Hit% · xwOBA (+ xSLG/xBA/wOBA · avg EV) | **HAVE (staging, unwired — operator-confirms coverage)** | NEW `deriveMlbStatcastQuality.js` (2026-06-18, v3) → `backend/data/mlbStatcastQuality.json`. Joins two Savant SEASON-AGGREGATE leaderboard CSVs — `/leaderboard/expected_statistics` (xwOBA/xBA/xSLG/wOBA; qualified base) + `/leaderboard/statcast` (barrel%←`brl_percent`, hard-hit%←`ev95percent`, avg EV←`avg_hit_speed`, batted balls←`attempts`) — current-ET-season (from `slateDate`). (v1 `statcast_search` event search → 0 coverage; v2 `exit_velocity_barrels` slug → empty; v3 `/leaderboard/statcast` is the CA-verified slug.) **Zero live consumer by design** — wire post-freeze + forward-CLV-gated. |
