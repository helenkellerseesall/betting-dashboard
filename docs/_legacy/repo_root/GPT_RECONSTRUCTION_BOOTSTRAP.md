...
| **Type** | Intelligence — NEW additive small-cap composite for sustainable hitter legitimacy (longshot-preserving) |
| **Bottleneck solved** | Existing composite (BC-2 depth × teamTotal 7% + OE-2 runEnv × teamTotal × carryShift 5% + OE-3/4/11/13) had no dimension for **lineupSpot precision (numeric 1-9)** or **stat-side coherence** (HR over from 9-spot + dead env = clearly random) or **model-trust alignment** (tiny modelProb + huge edge = "random variance spam" signature). PCE-1A adds these as a small-cap additive weight to make believable longshots feel EARNED while applying a small (bounded ≤ -0.04) penalty to clearly noise-driven survival. Operator-cemented constraints honored: DO NOT make "safe" / DO NOT remove longshots / DO NOT remove ladders — ONLY increase conviction quality + reduce fake-randomness. Hitter-overs-only (pitcher / under bypass cleanly). NEW `backend/pipeline/shared/playerConvictionEngine.js` (pure deterministic; no IO; no LLM; no fabricated sabermetric fields). Wired into `scoreCandidate` alongside BC-2/OE-2/OE-3/OE-4/OE-11/OE-13 (trust layers preserved verbatim). 5 NEW PCE canonical signal ids in `bettorLanguage.js` + reasonTag→signal-id helper. `compactPlay` surfaces `convictionNote` + `convictionReasonTag`; FE `FeaturedCard.tsx` renders bettor-readable `◆ {convictionNote}` line. NEW `verifyPlayerConvictionEngine1A.js` (53/53 PASS) enforces additive-only / canonical-only-fields / no-fabricated-sabermetric-refs / weight-band / longshot-preservation / determinism. `ops:verify`: 37/37 PASS. |

Prior phase (Candidate-Ecology-Parity-1A): NEW `verifyCandidateEcologyParity.js` restored MLB battlefield widening (date-sanity gate) + NBA slip ecology (aggressive + lotto NBA tier overrides); both preserve trust layers.
...
| Field | Value |
|---|---|
| File version | Player-Conviction-Engine-1A (2026-05-17) |
| Total approved phases | 31 (Realism-1A through Player-Conviction-Engine-1A) |
| Total verifiers | 31 (`verify*.js` in `backend/scripts/`) |
