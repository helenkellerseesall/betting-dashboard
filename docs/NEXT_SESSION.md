...
_Last updated: 2026-05-09 (Session Z: NBA Ecology Constitution Audit — 20-section audit complete; NBA health 2.9/10; 7-phase evolution roadmap NBA-1→NBA-7 defined; no code changes)_
...
| **NBA-1 — PRA volatility fix** | ⬜ NEXT | Path A: snapshot-sourced volatility field |
| **NBA-2 — Canonical path designation** | ⬜ BLOCKED on NBA-1 | designate buildNbaAiSlips as workstation canonical |
...
**Repo health: 7.2/10** structural. NBA intelligence health: **2.9/10** (audited). Primary evolution lever: NBA-1+NBA-2.
...
# 1. Finalize checkpoint — commits Sessions H–Y
bash scripts/finalizeCheckpoint.sh
# → Report the commit hash

# 2. REQUIRED: Restart TERM 1
#    server.js (mutex fix) + workstationRoutes.js (compactor import) both changed
...
### 🔴 Priority 1 — NBA-1: Fix PRA volatility / lotto starvation (Path A)

**Context from Session Z audit**: PRA at base odds dec ~5–9 is classified `aggressive` by `VOLATILITY_RULES` because `normFam.includes("combo") || normFam === "pra"` → maps to aggressive. But nightly pipeline stamps `volatility: "lotto"` on PRA snapshot candidates that qualify. Path A: use that field directly when `bet.snapshotSourced === true`.

**Prerequisite**: Opus audit of `classifyVolatility` override chain before touching anything. Trace: `buildFeaturedPlays.normalizeCandidate()` → `classifyVolatility()` → `buildPortfolioOptimizer.VOLATILITY_RULES`. Confirm `bet.snapshotSourced` flag exists and `volatility` field is present on snapshot candidates.

```javascript
// Path A — in buildFeaturedPlays.normalizeCandidate() or buildSlipAi.normalizeCandidate():
// BEFORE: volatility: classifyVolatility(raw)
// AFTER:
if (raw.snapshotSourced && raw.volatility) {
  volatility = raw.volatility; // use pipeline-stamped field directly
} else {
  volatility = classifyVolatility(raw); // fallback for non-snapshot bets
}
```

**After fix**: verify MLB ecology unchanged. `VOLATILITY_RULES` must not be modified — only the guard changes.

**Model: Opus** — audit before touching.

---

### 🔴 Priority 2 — NBA-2: Designate buildNbaAiSlips as canonical workstation path
...
| NBA lotto slip starvation | classifyVolatility override chain — audit before touching normalizeCandidate. Path A: snapshotSourced guard only. |
| classifyVolatility affecting MLB | Any VOLATILITY_RULES change must verify MLB ecology unchanged. VOLATILITY_RULES must not be modified — only the guard changes. |
...
**Session Z: TERM 1 restart: NO**
- Session Z was read-only (NBA Ecology Constitution Audit). No files modified. No restart required.

**Pending from Session Y (if not yet done): TERM 1 restart: YES**
- `server.js` modified (mutex unified — `/refresh-snapshot` now uses module-level `__refreshInProgress`)
- `workstationRoutes.js` modified (compactor import added)
