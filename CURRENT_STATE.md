...
> **2026-05-17 PHASE Operational-Parity-1A:** canonical `ops:*` wrappers restored to FULL historical orchestration depth. Operator caught `ops:term2` finishing in ~5s vs historical ~60s+; COS-1C wrappers were inline `&&` chains that silently dropped Verification Telemetry V1 + slate/market/lineage status helpers + git checkpoint stages. NEW orchestrators under `backend/scripts/ops/` (runTerm2Workflow / runCheckpointSeal / showTerm1Status) chain the historical depth verbatim. NEW `verifyOperationalParity.js` (69/69 PASS) enforces — wrappers MUST preserve historical orchestration depth. `npm run ops:verify` now runs 34/34 checks (1 runtime + 28 verify\*.js + 5 probes) in single command.
